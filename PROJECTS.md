...
Review date: 2026-09-24. 16 public repositories.
...
| 项目与入口 | 实现重点 | 验证与边界 |
| --- | --- | --- |
| [screenweaver](https://github.com/GOOD-123-CPU/screenweaver) · [架构](https://github.com/GOOD-123-CPU/screenweaver/blob/main/docs/architecture.md) | 配置驱动可视化引擎；数据源生命周期、组件注册与渲染分层。 | HTTP 状态/取消/轮询重叠测试，配置校验、单测、应用与库构建 CI |
| [itinera](https://github.com/GOOD-123-CPU/itinera) · [行程引擎](https://github.com/GOOD-123-CPU/itinera/blob/main/src/lib/itinerary.ts) | AI 行程产品；结构化输出解析、时间/坐标语义校验、候选 ID 约束与确定性兜底。 | 语义约束与候选绑定单测；预订动作回绑数据库 canonical record；构建 CI |
| [medirag-open](https://github.com/GOOD-123-CPU/medirag-open) · [系统架构](https://github.com/GOOD-123-CPU/medirag-open/blob/main/docs/ARCHITECTURE.md) | Java 医疗知识问答；混合召回、RRF、重排序、置信度兜底、引用与 SSE。 | 后端/前端/密钥/依赖审计 CI；有 qrels 才报告 Recall/MRR，无 qrels 仅报告 proxy 指标 |
...
| [OpenInterview](https://github.com/GOOD-123-CPU/OpenInterview) · [架构](https://github.com/GOOD-123-CPU/OpenInterview/blob/main/docs/architecture.md) | 语音面试应用；简历、出题、转录、报告 worker，以及 SQLite 原子 task lease。 | 62 pytest；Python 3.10/3.11/3.12 CI；多 worker 互斥与过期 lease 恢复测试；AI 结果需人工复核 |
...
| 项目与入口 | 实现重点 | 验证与边界 |
| --- | --- | --- |
| [hanbayes](https://github.com/GOOD-123-CPU/hanbayes) · [Python API](https://github.com/GOOD-123-CPU/hanbayes/blob/main/src/hanbayes/analyzer.py) | 可解释贝叶斯分类包；共享稀疏特征、模型对比、冻结配置与统计检验。 | 跨 OS/Python CI；AUC/McNemar/bootstrap 单测；冻结配置与已发布结果 artifact contract 检查 |
...
| 项目与入口 | 实现重点 | 验证与边界 |
| --- | --- | --- |
| [LexAtlas](https://github.com/GOOD-123-CPU/LexAtlas) · [架构](https://github.com/GOOD-123-CPU/LexAtlas/blob/main/docs/ARCHITECTURE.md) | 法律知识检索应用；RAG、文档入库、缓存、管理功能与安全兜底。 | Java 与 Vue 测试/构建 CI；fallback 状态持久化有单测；领域效果仍需独立评估 |
...
| [investment-committee](https://github.com/GOOD-123-CPU/investment-committee) · [研究流水线](https://github.com/GOOD-123-CPU/investment-committee/blob/main/src/lib/pipeline/runner.ts) | Next.js 投研终端；确定性量化核心、专业智能体、牛熊辩论、风险审查与 CIO 汇总。 | Bun 确定性核心单测 + Prisma/ESLint/生产构建 CI；模型辩论本身不代表投资效果 |
